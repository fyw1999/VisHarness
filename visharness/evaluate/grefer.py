...
class G_REFER:
    ...
    def showRef(self, ref, seg_box='seg'):
        ...
        # gRefCOCO modification: handle multi-target (list) and no-target ([-1])
        ann_ids = ref['ann_id']
        if not isinstance(ann_ids, list):
            ann_ids = [ann_ids]

        if ann_ids == [-1] or len(ann_ids) == 0:
            print("Notice: No visual target for this query (No-target).")
            return
        ...
        if seg_box == 'seg':
            for ann_id in ann_ids:
                ...
                if type(ann['segmentation'][0]) == list:
                    # polygon used for refcoco*
                    for seg in ann['segmentation']:
                        # Fix for Python 3: use integer division //
                        poly = np.array(seg).reshape((len(seg)//2, 2))
                        polygons.append(Polygon(poly, True, alpha=0.4))
                        color.append(c)
                    p = PatchCollection(polygons, facecolors=color, edgecolors=(1,1,0,0), linewidths=3, alpha=1)
        ...
        # show bounding-box
            ...
            bboxes = self.getRefBox(ref['ref_id']) # Returns a list of boxes: [[x,y,w,h], ...]
            for bbox in bboxes:
                box_plot = Rectangle((bbox[0], bbox[1]), bbox[2], bbox[3], fill=False, edgecolor='green', linewidth=3)
                ax.add_patch(box_plot)
